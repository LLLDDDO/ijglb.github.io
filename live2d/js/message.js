...
var renderer = new PIXI.WebGLRenderer(250, 280,{transparent: true});
document.getElementById('live2d').appendChild(renderer.view);
var stage = new PIXI.Container();
var live2dSprite = 0;
var clickTime = 0;
var broken = !1;
var brokenModel = null;
function animate() {
    requestAnimationFrame(animate);
    renderer.render(stage);
}
animate();
function showbyPixi(modelPath){
    $.ajax({
        cache: true,
        url: modelPath,
        dataType: "json",
		success: function (result) {
			var cdn = "https://cdn.jsdelivr.net/gh/ijglb/ijglb.github.io@latest/";
			//修改model 接入cdn
            ...
            live2dSprite = new PIXI.Live2DSprite(result,{debugLog: true});
            ...
            live2dSprite.on('click', (evt) => {
                ...
                if (live2dSprite.hitTest(null, point.x, point.y)) {
                    ...
                    if(!broken && clickTime > 10){
                        ...
                        broken = true;
                        hitFlag = true;
                        showMessage("变态！衣服都被你搞破了…TAT", 5000);
                        setTimeout(function(){
								hitFlag = false;
						},5000);
                    }else{
                        live2dSprite.startRandomMotionOnce('click',3);
                    }
                } 
            });
            showLive2d();
...
$(document).ready(function() {
	var modelArr = ["kp31_310","kp31_1103","rfb_1601"];
	var model = modelArr[Math.floor(Math.random() * modelArr.length + 1)-1];
    brokenModel = message_Path+"model/"+model+"/destroy/model.json";
    showbyPixi(message_Path+"model/"+model+"/normal/model.json");
    initLive2d();
});
}else{
  window.live2d = true;
